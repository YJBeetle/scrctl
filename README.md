...
- 屏幕流 **58.9 – 61.1 fps**（面板 60Hz），解码链路两后端可选：VideoToolbox 硬解，
  以及**遇到超过 64KB 的关键帧时自动切到 libavcodec 软解**（不丢帧、不重起会话）。
  同一份录屏两后端都 412/412 出帧，同帧逐像素平均绝对差 0.92
...
ffmpeg 是**可编译可不编译**的（`-DSCRCTL_LIBAV=OFF` 可关掉），但强烈建议留着：
真机主屏的关键帧实测有 49652~70101 字节，而 VideoToolbox 的长度前缀只有 2 字节可用
（上限 65535），没有软解后端时这种码流只能整帧丢、并且因为流不周期发 IDR 而**永久
灰屏**。非 Apple 平台没有 VideoToolbox，软解就是唯一后端。
