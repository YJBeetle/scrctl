...
// "此刻有一条流活着"。
//
// 为什么要问这个：实测设备会在最后一个视频包之后约 6.9 秒把整条媒体会话结束掉
// （docs §13）。如果那条门是跟着会话生死走的，那么自动化框架里"屏幕静止了一会儿
// 之后再点一下"这个最常见不过的时序，点下去会被 backboardd 静默丢掉——而 send 是
// 只发不收的，这边毫无感觉。这比"截图截到旧帧"严重得多。
//
...
int main(int argc, char **argv) {
    ...
    for (;;) {
        ...
        if (quiet >= 9000) {
            log(t0, "已静默 " + std::to_string(quiet) + "ms（实测拆流点 6.9s），判定会话已被设备结束");
            break;
        }
    }
