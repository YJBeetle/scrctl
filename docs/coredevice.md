...
`stopmediastream` 不报错，也不需要额外等待。所以用户说的"点下去愣一下"几乎全在
发现延迟上：原来的 wake 路径先问一次会话状态（一条 RPC，100–300ms）再决定要不要
重起，等于把手感里最大的一笔开销花在确认一个本来就打算处理的事实上。现在改成
**用户动手且已静默超过拆流点（7.5 秒）就直接重起**；静默没到这个数时会话其实还
活着，用户这一下本身就会把画面推过来，那时重起纯属捣乱——上一版用的 150ms 就是这个
错误，它让每一次按下都撞上一整轮"停+起+全量 IDR"，延迟叠加到用户看到慢动作（见
§13 末尾那条复盘）。静默到点那条路径仍然先 probe，因为那里要的是"别在流活着的时候
白重起"。

**wake() 到第一帧的实测预算：192–229ms，中位 206ms。** `tools/wake_latency_probe`
把 silence/stall 自动重起都关掉，等包计数静默满 9 秒（也就是会话确实已被设备结束），
然后调一次 `wake()` 并等第一帧，5 次全部成功。这笔时间花在"等泵线程从 50ms 的收包
超时里转出来"加上停+起加设备吐出第一个 IDR。**为什么专门量它**：任何按"截图 -> 动作
-> 再截图"跑的调用方都会给等帧设一个固定超时，而这个数就是超时该设多大的依据——
设小了，重起还没回来它就退回旧帧，调用方拿到的是拆流前那一刻的画面，却以为是动作
之后的。取帧方（MaaFramework 的 iOS 控制单元）据此把预算定在 800ms，约是最大值的
3.5 倍。
