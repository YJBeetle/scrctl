...
**这一句 `wake()` 有没有用，是用对照验的，不是靠推理。** 同一个驱动跑两遍"连接 ->
截图 -> 静置 20 秒 -> swipe 画一条线 -> 再截图"，两遍画在互不重叠的纵向带上：

```text
有 wake()：  收到操作：设备已结束这条流，重起媒体会话
             两张图差异 1030 个像素        -> 拿到的是动作之后的画面
无 wake()：  （没有任何重起发生）
             两张图差异    0 个像素        -> 停在旧帧上
```

差异 0 就是"截图返回真、内容却是陈的"那种错法的形状。少了下面那一遍，上面那一遍
什么都证明不了。
