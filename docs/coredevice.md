...
**画面静止约 3–5 秒后，设备会自己把这条流整个结束掉**（不是"暂时不发"）。判据是
...
**发 RTCP 不能阻止它拆流，反而会把投递打死。** 协商参数里写着
`RTCPSendInterval: 1` / `RTCPTimeoutInterval: 20`，很自然地会猜"是不是我们不回
接收报告设备才结束流"。A/B 测下来结论相反：

| 做法 | 25 秒内"设备已结束这条流" | 解码帧数 |
| --- | --- | --- |
| 不发 RTCP | 4–5 次 | 426–812（含转场动画） |
| RR 发到视频端口（RTCP 与 RTP 同端口） | **0 次** | **不到 60** |
| RR 发到 `sender_port + 1` | 4 次 | 545 |

发到视频端口时会话确实不再被结束，但设备**几乎不再推帧**——连按 home 换画面都收
不到，也就是被我们一个格式正确的 RR（`0x81 201 len=7` + sender/media SSRC + 报告
块）打死了；发到奇数端口则完全没影响，说明设备不在那儿听。三条一起看：**这条流没有
可用的 RTCP 通道**，空闲拆流是设备策略，拦不住，只能靠"发现死了就重起 + 用户一动就
立刻催一次检查"。
