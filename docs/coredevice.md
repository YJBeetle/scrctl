...
`allowRTCPFB`)"），闸关着时测出来的"不理"是必然的。所以成立的结论只是："在填错 SSRC、
PT=206、且没有申报 RTCP 反馈能力这三件事同时成立时，设备不响应关键帧请求。"
`tools/fir_probe` 已改成这三个变量各自可控（臂名后缀 `m`、臂 `fir205`、`+fb`），并且把
"等画面静止"的截止卡在租期内——租期一改大，这一类实验第一次有了不受拆流干扰的观察窗。
