...
**这条流是"起流后约 20 秒的硬租期"，不是空闲超时。** 之前这一节写的是"画面静止 6.9
...
**"到点就接续"因此改成"到点之后挑一个静止的间隙接续"。** 这笔钱非付不可（租期不延长），
能选的只有时刻：画面在动时接续是看得见的一次顿挫，画面静止时接续是免费的（显示的那一帧
本来就停在那里，新会话回来的第一帧和它一模一样）。所以 `kSessionLeaseMs = 18000` 之后还
有 `kSessionLeaseHardMs = 19400`——18 秒到点先等着，视频档一静默（`kRenewQuietMs = 1000`）
就立刻接，等不到就在 19.4 秒硬接。催流那条路反过来：只有过了设备那 20 秒（`kSessionLeaseDeadMs`）
才直接重起，18~20 秒这段会话**还活着**，手上这一帧就是用户这一下要的帧，不能因为"快到点了"
扔掉。
