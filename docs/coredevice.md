...
RemoteXPC 握手都超时、只有重启设备能救"）。所以"p3 有保活所以不断"这个印象的来源，是它的
看门狗把断流变成了用户不太会注意到的约 300ms 卡顿——每 20 秒一次。连 Apple 自己那条 RCTL
反馈通道也不续命：8 秒租期那一臂发了 641 个 RTCP（约 80/s，含每帧一个的伴随包），仍死在
+7.996s。
