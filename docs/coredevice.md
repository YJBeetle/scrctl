...
**所以这条镜像流的预算就是设备定的 6 Mbps，而它会用帧率去换码率。** 跑游戏时量到
手机自己屏幕 60fps 很顺、镜像里却是慢动作：同一时刻设备发的码率始终 ~5.5Mbps，
每帧从 10 个包（11KB）涨到 50 个包（57KB），帧数就从 60 掉到 12——**保码率、降帧率**。
我们这一侧没有丢任何东西（`序号缺口 0`、`分片作废 0`、AU 数与解码数相等），
所以这不是管线 bug，是设备策略，而策略的两个入口（码率、分辨率）都没有旋钮。
要在这种场景拿到帧率，能动的只有"让每帧需要更少比特"这一类，而它恰好就是被锁住的
那个分辨率。
...
**用户动手且已静默 150ms 以上就直接重起**（会话其实还活着时重起也不亏，新会话必然
带一个干净关键帧）；静默到点那条路径仍然先 probe，因为那里要的是"别在流活着的时候
白重起"。
