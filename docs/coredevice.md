...
3. **FIR 不但没用而且有害**：它自己不换来 IDR，还**把续命废掉**——设备侧那条 socket
   `pkts in: 40`（包明明到了），而 `Last RTCP packet receive time:nan`、20.13s 准时死。
   同臂、同频、同 SSRC，唯一变量就是那个 FIR，所以是它让设备的 RTCP 解析放弃了更新。
   笔记里"it honors FIR (PT=206 FMT=4, requires allowRTCPFB)"**作废且方向相反**：
   **永远不要发 FIR**。
4. `allowRTCPFB` 那一位对以上都没有影响（`rrsrc+fb` 活、`fir` 不带 fb 也照样死）。
