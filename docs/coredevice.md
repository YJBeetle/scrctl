...
#### 把苹果的请求逐键对齐之后：七个假设连同数据一起作废，只剩一个结构性差异没控住
...
**目前唯一还没控住的结构性差异：苹果有音频腿。** 它先 `type:"audio"` 再 `type:"video"`，
两条**共用同一个 `avcMediaStreamOptionClientSessionID`**，而那条精确 1.000Hz、整场从不空档的
`RR+SDES` 是发在**音频腿**上的（客户端 52800 → 设备 54228，全场 74 个，间隔 1.000±0.001 秒）。
如果设备的计时器是按 `ClientSessionID` 记的、只被音频腿的 RTCP 复位，那所有观测都能对齐：
我们（和 p3）只有视频腿，所以报多少秒就多少秒死；苹果报了同样的 20，却永远到不了点。
p3 那边虽然写着 `start_audio_stream`，但它自己的会话表显示跑起来时**只剩视频一条**
（`+40s sessions=1`），也就是 p3 实际上也是视频腿独活——所以"p3 也断"**不能**否掉这条假设。
要判它得自己搭音频腿，见任务 #21。
