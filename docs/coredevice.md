...
**产品结论：既然续不上，就在到点之前自己接续。** `FramePump` 里 `kSessionLeaseMs =
18000`：镜像那条路到点主动重起（留 2 秒余量给 RPC 抖动），催流那条路发现租期已过就不
再判静默、直接重起。实测代价：静止画面上这次重起几乎是白的（新会话那个 IDR 本来就是一
帧新画面，静止时本来一帧都没有）；画面在动时是约 0.4 秒的掉帧（那一段 20 帧，前后都是
60 帧）。而它换掉的是"死了之后 1.2 秒起步的盲区 + 可能交回一张旧画面"——每 20 秒必然
发生一次，这就是用户报的"有时候会断"。
