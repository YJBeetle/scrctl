...
public:
    ...
    struct Stats {
        ...
        /// "画面静止"这个判断只看 video 那一把（见 last_video_ms_）。
    ...
    /// 告诉泵"用户刚刚动了"。
    ///
    /// 为什么需要：一条会话只有起流后约 20 秒的租期（`kSessionLeaseMs`，喂画面和回
    /// RTCP 都续不上，docs §13），而流死了之后设备上任何变化都不会再推过来。泵自己按"静默满
    /// silence_restart_ms"发现这件事，那是给"有人盯着窗口"的场景兜底用的；交互和
    /// 自动化要的是**动手/取帧的这一刻**就知道，所以用这条便宜的信号去催一次检查。
    /// 判断本身分三档，靠的是设备那条每秒一个的 RTCP SR 心跳：心跳还在就什么都不做，
    /// 可疑区间去问设备一句，静默到两个心跳以上直接重起（推导见 FramePump.cpp 里
...
private:
    ...
    /// 最后一个**视频包**（不是 SR 心跳）的时刻。和上面那把尺分开是必需的：设备的 SR
    /// 在画面完全静止时也照每秒发一个，所以"任何数据报的静默"最短也是 1 秒，拿它判
    /// "画面静止"会一次都判不到；而这一把尺在静止画面上会一直涨上去。租期接续就挑这
    /// 一把尺读出来的间隙做（见 FramePump.cpp 的 kRenewQuietMs）。
    uint64_t last_video_ms_ = 0;
