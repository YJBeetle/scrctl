...
/// **兜底**用的一张"编码分辨率 -> 可见区"表。主路径是问设备（
/// `remote::fetch_display_info`，feature 为 `displayinfoupdates`），只有问不到的时候
/// 才落到这里——所以它的正确用法是"问到 0 个字段时的最后一级"，不是几何的来源。
///
/// 为什么还需要最后一级：表里的这一档是真机量出来的（编码 1136x2464，可见区
/// 1125x2436，右 11px / 底 28px 是 HEVC 按 CU 对齐的填充），而设备侧的 feature 在
/// 某些 DDI 版本上不在、或者那条服务连接被别人占着。文件回放（`--play`）也没有设备
/// 可问，走的同样是这一级。
///
/// 不裁会有两个后果：画面边缘有一条噪声，以及**触摸坐标偏**——触摸面的 0..1 是相对
/// 可见区的，拿编码尺寸当分母会在右下方向错出 1% 左右。
///
/// 表外机型的处置是"整幅当可见区"：宁可留一条噪声边，也不要凭猜裁掉真内容。
/// 另一条路是读 SPS 的 conformance window（任何机型都不用列数字），还没做。
