...
struct Offer {
    ...
    ///
    /// 后果要记清楚：预算锁死 6 Mbps，画面一复杂设备就用帧率换码率（游戏场景实测
    /// 60fps→12fps，每帧从 11KB 涨到 57KB），而单帧能长到 70101 字节，VideoToolbox
    /// 只吃 2 字节长度前缀（上限 65535）——所以"关键帧大到喂不进去"是常态风险，
    /// 只能靠软解后端兜，见 decode/Decoder.h 的 create_software_decoder。
};
